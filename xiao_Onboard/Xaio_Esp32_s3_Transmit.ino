...
constexpr float   LORA_FREQ_MHZ     = 915.0;  // match receiver frequency
constexpr uint32_t SEND_INTERVAL_MS = 1000;   // target: 1 data point per second
...
bool gpsTestRunning = true;   // start by streaming test points until told to stop
int gpsTestIndex = 0;
bool gpsEnabled = true;       // gate GPS usage so we only poll when desired
unsigned long lastSendMs = 0; // non-blocking scheduler for send cadence
...
void setup() {
  ...
  // start sending test points immediately; send GPS_STOP to halt
  gpsTestRunning = true;
  gpsTestIndex = 0;
  lastSendMs = millis();
  Serial.println("Auto-starting GPS test stream (send GPS_STOP to end, GPS_TEST to restart)");
}
...
void runCommand(String cmd) {
  ...
  if (cmd == "LED_ON") {
    ...
    gpsEnabled = false;
    Serial.println("GPS disabled; no live fixes will be sent");
  }
...
void checkForCommands() {
  // Use a short timeout so we don't block the 1 Hz transmit loop
  String incoming = "";
  int state = radio.receive(incoming, 0.05f); // ~50 ms timeout
...
void loop() {
  ...
  while (gpsEnabled && GPSSerial.available()) {
  ...
  if (!gpsEnabled) {
    // GPS disabled via command; keep listening for commands but skip fixes
    delay(5);
    return;
  }

  // If GPS has good data, send telemetry
  if (gps.location.isValid() && gps.location.age() < 2000 && (now - lastSendMs >= SEND_INTERVAL_MS)) {
